...
# MAGIC 
# MAGIC ## Instructions:
# MAGIC 1. Run the notebook against a cluster with Databricks Runtime version 9.1.x, to best re-create the training environment.
...
model_name = "diabetes-demo-1"

# COMMAND ----------

# MAGIC %md
# MAGIC ## Environment Recreation
# MAGIC Run the notebook against a cluster with Databricks Runtime version 9.1.x, to best re-create the training environment.. The cell below downloads the model artifacts associated with your model in the remote registry, which include `conda.yaml` and `requirements.txt` files. In this notebook, `pip` is used to reinstall dependencies by default.
# MAGIC 
...
# MAGIC %md #### Download model dependency to be able to install via pip
...
model_name = "diabetes-demo-1"
input_table_name = "default.iris100x"
output_table_path = "/FileStore/batch-inference/diabetes-demo-1"
